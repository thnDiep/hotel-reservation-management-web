...
const PlaceItem = ({ item, liked }) => {
    ...
    const renderSlides = item.HinhAnh.map((image, index) => (
        <div className="image" key={index} onClick={() => handleNav(`/hotels/detail/${item.ID}`)}>
    ...
    return (
        <div className="place">
            <div className="place__img">
                ...
                <HeartButton liked={liked} IDKhachSan={item.ID} />
            </div>
            <div className="place__detail" onClick={() => handleNav(`/hotels/detail/${item.ID}`)}>
                ...
                <div className="place__detail--price">
                    ...
                    {item.voucher && (
                        <div className="price__voucher">
                            <div className="price__voucher--name">
                                <span>Mã:</span>
                                <div className="price__voucher--title">{item.voucher.MaKhuyenMai}</div>
                                <span className="price__voucher--con">-{item.voucher.PhanTramKM}%</span>
                            </div>
                            <div class="price__voucher--ap">
                                {formatMoney(item.GiaMoi - (item.GiaMoi * item.voucher.PhanTramKM) / 100)}{' '}
                            </div>
                        </div>
                    )}
                </div>
